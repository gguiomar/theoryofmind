...
# Create figure with more space at the top for idea density
fig = plt.figure(figsize=(12, 9))
ax = plt.subplot(111)

# Adjust subplot to leave room at the top for idea density
...
ax.text(-1.2, len(row_labels) + 0.5, 'Idea\nDensity', 
...
for i, cat in enumerate(categories_ordered):
    ...
    ax.text(i + 0.5, len(row_labels) + 0.5, f'{idea_dens:.3f}', 
            transform=ax.transData,
            ha='center', va='center', 
...
# Extend y-axis to show idea density values at the top
...
plt.tight_layout()
plt.savefig('idea_density_heatmap_seaborn.png', dpi=300, bbox_inches='tight')
plt.show()

# Create the correlation plot with seaborn styling
...
# Use crest color palette for bars
bar_colors = sns.color_palette("crest", len(correlations))
...
ax.set_xlabel('Subject', fontsize=16, fontweight='bold')
...
plt.tight_layout()
plt.savefig('idea_density_correlation_seaborn.png', dpi=300, bbox_inches='tight')
plt.show()
